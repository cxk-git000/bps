...
class PostgreConnector:
    ...
    def fetch_benchmark_data(
        ...
        if version:
            ...
            filters.append(f"_m.insert_time >= '{timestamp_range_start}'")
            filters.append(f"_m.insert_time <= '{timestamp_range_end}'")
        ...
        select_parts = (
            ...
            "_m.insert_time,",
            "LAST_VALUE (value)",
            "OVER (PARTITION BY test.name ORDER BY _m.insert_time RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) last_value",
            "FROM benchmark.metrics as _m",
            "LEFT JOIN benchmark.project_version as pv ON _m.project_version_id = pv.id",
            "LEFT JOIN benchmark.hardware as h ON _m.hardware_id = h.id",
            "LEFT JOIN benchmark.backend as bk ON _m.backend_id = bk.id",
            "LEFT JOIN benchmark.branch as b ON b.id = _m.branch_id",
            "LEFT JOIN benchmark.test as test ON test.id = _m.test_id",
            "LEFT JOIN benchmark.parameters as p ON p.id = _m.parameters_id",
        )

        sql_string = format(
            "{} WHERE {} GROUP BY test, bit_size, value, _m.insert_time ORDER BY _m.insert_time DESC"
