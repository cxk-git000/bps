...
/// 执行盲旋转并从旋转后的 GLWE 密文中提取索引 0 和 1 的两个相邻 LWE 样本。
///
/// # 参数
/// - `msed_input`: 经过模数切换后的输入 LWE 密文。
/// - `lut`: 存储查找表的 GLWE 密文，将被盲旋转修改。
/// - `fourier_bsk`: 傅里叶域引导密钥。
///
/// # 返回值
/// 返回两个 LWE 密文，分别对应索引 0 和 1 位置的系数。
/// 这是 SDR-PBS Shared-input Dual-Recovery Programmable Bootstrapping 需要的固定提取位置。
pub fn blind_rotate_and_extract_two_coefficients<OutputScalar, OutputCont, KeyCont>(
    msed_input: &impl ModulusSwitchedLweCiphertext<usize>,
    lut: &mut GlweCiphertext<OutputCont>,
    fourier_bsk: &FourierLweBootstrapKey<KeyCont>,
) -> (LweCiphertextOwned<OutputScalar>, LweCiphertextOwned<OutputScalar>)
where
    OutputScalar: UnsignedTorus,
    OutputCont: ContainerMut<Element = OutputScalar>,
    KeyCont: Container<Element = c64>,
{
    // 1. 执行盲旋转
    blind_rotate_assign(msed_input, lut, fourier_bsk);

    // 2. 准备两个输出 LWE 密文
    let output_lwe_dim = fourier_bsk.output_lwe_dimension();
    let ciphertext_modulus = lut.ciphertext_modulus();

    let mut sample0 = LweCiphertext::new(
        OutputScalar::ZERO,
        output_lwe_dim.to_lwe_size(),
        ciphertext_modulus,
    );
    let mut sample1 = LweCiphertext::new(
        OutputScalar::ZERO,
        output_lwe_dim.to_lwe_size(),
        ciphertext_modulus,
    );
    ...
    extract_lwe_sample_from_glwe_ciphertext(lut, &mut sample0, MonomialDegree(0));
    extract_lwe_sample_from_glwe_ciphertext(lut, &mut sample1, MonomialDegree(1));
